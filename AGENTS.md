...
If you're an internal contributor, when creating a new PR, branch off litellm_internal_staging and use a branch prefixed with litellm_. Do not put `/` in branch names

Do not add assistant attribution to commit messages, PR descriptions, or comments, including `Co-Authored-By` and generated-by lines. Do not create a new PR or branch for a change that belongs on the current PR branch.
